...
image:{PAGES_URL}/images/project_with_junit4ks_injected.png[]
...
The `CalculatorTest.groovy` is a unit-test for the custom keyword `Calcurator.groovy`. You can run the unit-test by running the `CalcluratorTestRunner` with by clicking the gree button image:https://kazurayam.github.io/junit4ks/images/run_katalon_test.png[green button].
...
But I realized that providing only jar is not enough. When I want to apply the `junit4ks` in any Katalon Studio project of my own, I always needed to look back the link:{REPOS_URL}[project repository] for sample codes. I needed to copy & paste the `Test Cases/test/CalcularatorTestRunner` and the `Include/Include/scripts/groovy/junittutorial/CalculatorTest.groovy` because I always forget how to write the codes that utilizes `junit4ks`. Therefore I thought I need to develop new way of distributing the `junit4ks`.

I extended the link:{REPOS_URL}/build.gradle[build.gradle] so that it implements a new task `createDist`. In the `junit4ks` project I do this:
